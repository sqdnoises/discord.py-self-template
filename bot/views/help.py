...
class HelpView(discord.ui.View):
    ...
    async def update_pages(self) -> None:
        self.page = 1
        self.pages = utils.paginate([command for command in self.bot.commands if command.cog == self.category and await command.can_run(self.ctx) and not command.hidden], self.items)
    ...
    async def first_page(self, interaction: discord.Interaction) -> None:
        self.page = 1
        embed = await craft_help_embed(self.ctx, self.category, self.page, self.items)
